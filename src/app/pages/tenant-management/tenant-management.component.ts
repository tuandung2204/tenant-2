...
export class TenantManagementComponent implements OnInit {
  ...
  createTenant() {
    if (!this.newTenant.clinicName || !this.newTenant.clinicId) {
  ...
  deleteTenant(clinicId: string) {
    this.tenantService.deleteTenant(clinicId).subscribe({
      next: () => {
        this.tenants = this.tenants.filter(tenant => tenant.clinicId !== clinicId);
  ...
  updateTenant() {
    ...
    this.tenantService.updateTenant(this.newTenant).subscribe({
      next: (updatedTenant) => {
        ...
        const index = this.tenants.findIndex(t => t.clinicId === this.newTenant.clinicId);
        if (index !== -1) {
